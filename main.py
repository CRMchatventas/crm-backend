...
async def api_consultar_precio(nombre: str, consola: str = "", vendedor_id: str = "anonimo"):
    print(f"\n🏷️ [RADAR PRECIOS] Buscando: {nombre} ({consola})")
    ...
    html_search = await obtener_html_escalonado_async(url_search)
    if not html_search: return {"status": "error_precio_cero", "detalle": "Error Radar", "nombre_corregido": nombre, "url_pc": url_search, "mxn": {"loose": 0, "cib": 0, "new": 0}}
    ...
    if p_loose == 0 and p_cib == 0:
        print(f"⚠️ [RADAR] No encontrado: {nombre}. Devolviendo $0 para revisión Godot.")
        return {
            "status": "warning_cero",
    ...
    print(f"✅ [RADAR EXITO] Nombre Oficial: {nombre_oficial_pc} | CIB: ${mxn_cib_real}")
    return {
        "status": "ok",
        "nombre_corregido": nombre_oficial_pc,
